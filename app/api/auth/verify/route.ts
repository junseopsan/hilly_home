...
export async function POST(request: NextRequest) {
  try {
    ...
    const { apiToken, apiCertNum } = body;

    console.log("받은 파라미터:", { apiToken, apiCertNum });

    // 파라미터 유효성 검사
    if (!apiToken || !apiCertNum) {
      console.log("파라미터 누락:", { apiToken, apiCertNum });
...
// KMC 본인인증 API 호출 함수
async function verifyWithKMC(
  apiToken: string,
  apiCertNum: string
): Promise<AuthResult> {
  try {
    console.log("KMC API 호출 시작:", { apiToken, apiCertNum });
    ...
    console.log("KMC API 응답 상태:", response.status);

    if (!response.ok) {
      console.log("KMC API 호출 실패:", response.status, response.statusText);
      return {
        success: false,
        message: `KMC API 호출 실패: ${response.status} ${response.statusText}`,
      };
    }

    const result = await response.json();
    console.log("KMC API 응답:", result);
    ...
    if (result.result_cd === "APR01") {
      ...
      switch (result.result_cd) {
        ...
        case "APR06":
          ...
          break;
        default:
          errorMessage = `오류 - 알 수 없는 오류 (${result.result_cd})`;
      }
    ...
    return {
      success: false,
      message: `본인인증 검증 중 오류가 발생했습니다: ${error instanceof Error ? error.message : "Unknown error"}`,
    };
  }
}
